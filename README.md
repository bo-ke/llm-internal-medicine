...
| 10 | `index_exactness` | `qk_stats/.../index_exactness` | `live_keys / scanned_span` | 每层+全局 | 仅 CSA/HCA 层；1.0 表示统计的 key 集合与真实一致 |
| 11 | `attn_sink_logit` | `qk_stats/.../attn_sink_logit` | `mean(attn_sink)` | 每层+全局 | 仅 CSA/HCA 层；learned sink logit 量级漂移 |
...
- `index_exactness`：真实 key 数 / kernel 扫描区间长度。`1.0` 表示区间表示无损；`< 1.0`
  出现在 learned indexer 场景（压缩集合在区间内稀疏），此时统计覆盖真实 key 的超集，
  按近似值解读。
...
| **QK** | `index_exactness` | `live_keys / scanned_span` | mean | CSA/HCA 层；< 1 表示统计为近似 |
