...
L 是低于阈值的项数，`s ∈ [0,1]` 是参考值，取论文建议的 `s = φ(x)_min`。
指数项里 `(s−p)/s − 1 = −p/s`，配合 `s = φ_min` 得到

```
p_l / s = exp(x_l − x_min)      →      S(x) = (1/L) · Σ exp(−exp(x_l − x_min))
```

这个等价形式**只用 logits**，因此在 LM head 的词表规模下（fp32 里 `φ_min` 会下溢成 0）
仍然稳定。**上升 = 无关项被压得更接近最小值 = 更稀疏**，上界 `e⁻¹ ≈ 0.368`（所有低于
阈值的项都正好等于最小项）。
