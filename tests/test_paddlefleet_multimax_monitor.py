...
class DistributionMetricsTest(unittest.TestCase):
    ...
    def test_relevant_count_tracks_the_eps_partition(self):
        # relevant_count survives as the eps sanity check even though the metric
        # that used it as a denominator (the eps-thresholded Def 3.2 mean) was
        # dropped for being ~1 - top1_prob at the default eps.
        logits = paddle.to_tensor([[2.0, 1.0, -30.0]], dtype="float32")
        m = mm_metrics.compute_distribution_metrics(logits, prob_eps=1e-6, topk=1)

        self.assertAlmostEqual(float(m["relevant_count"]), 1.0, places=6)
        self.assertAlmostEqual(float(m["sparse_count"]), 1.0, places=6)
        self.assertNotIn("multi_modality", m)  # only the top-k form is reported
    ...
    def test_geomean_reference_is_the_baseline_tail_geometric_mean(self):
        # Default reference: the geometric mean of the *baseline* (unmodulated)
        # tail probabilities, with the tail selected by the baseline's own eps.
        logits = paddle.to_tensor([[6.0, -1.0, -2.0]], dtype="float32")
        ...
        # baseline tail == entries below 0.01 of the baseline's own mass
        tail = [float(v) for v in ref_p if float(v) < 0.01]
        self.assertTrue(tail, "the fixture must give the baseline a tail")
        s = math.exp(sum(math.log(v) for v in tail) / len(tail))
        expected = sum(math.exp(-float(p[i]) / s) for i in (1, 2)) / 2
        self.assertAlmostEqual(float(m["sparsity"]), expected, places=5)
        self.assertNotIn("min", mm_metrics.SPARSITY_REF_MODES)
    ...
    def test_quantiles_ignore_rows_the_mean_drops(self):
        # Row 1 has every non-max entry strictly at/below nothing -- it has no
        # entry below eps, so it is excluded from sparsity. The quantiles must be
        # over the surviving row only, not over the +inf sentinel that the mask
        # parked at the end of the sort.
        logits = paddle.to_tensor([[5.0, 0.0, -30.0], [1.0, 1.0, 1.0]], dtype="float32")
        m = mm_metrics.compute_distribution_metrics(logits, prob_eps=1.0 / 3, topk=1)
        single = mm_metrics.compute_distribution_metrics(logits[0:1], prob_eps=1.0 / 3, topk=1)
        self.assertAlmostEqual(float(m["sparse_count"]), 1.0, places=6)  # row 1 gives L = 0
        self.assertAlmostEqual(float(m["sparsity"]), float(single["sparsity"]), places=5)
        for suffix in ("p50", "p95", "p98"):
            self.assertAlmostEqual(
                float(m[f"sparsity_{suffix}"]),
                float(single["sparsity"]),
                places=5,
    ...
    def test_multi_modality_topk_separates_a_flat_head_from_a_peaked_one(self):
        # This is why the eps-thresholded mean was dropped: with eps = 1/V its
        # relevant set is a long tail whose probabilities are negligible next to
        # phi_max, so it collapses onto 1 - top1_prob. The top-k form stays
        # sensitive to the shape of the head itself.
    ...
    def test_sparsity_ref_modes_are_ordered_and_geomean_has_resolution(self):
        # uniform (s = 1/V) saturates near 1; the geomean reference sits in the
        # middle of the baseline tail, so the smooth step actually has resolution.
        paddle.seed(0)
        ...
        self.assertGreater(scores["uniform"], 0.9)  # saturated, hence not the default
...
class MultiMaxMonitorTest(unittest.TestCase):
    ...
    def test_unfused_path_emits_every_declared_metric(self):
        ...
        for name in ("entropy", "entropy_norm", "top1_prob", "top5_prob", "multi_modality_top5", "sparsity"):
